...
public class Empservice extends Employees2 {
	...
	public synchronized void readFile() {
		String file="D:\\Java\\employee.txt";
		...
		try {
			...
			while((l=b.readLine())!=null) {
				...
				String dept=arr[4];
				e=new Employees2(age,name,dept,e.id,salary);
				emplist.put(e.id, e);
	...
	public synchronized void writeTofile() {
		...
		String s="";
		try {
			File file=new File("D:\\\\Java\\\\employee.txt");
if(!file.exists()) {
	file.createNewFile();
}
FileWriter fw=new FileWriter(file);
b=new BufferedWriter(fw);
for(Map.Entry<Integer,Employees2>entry:emplist.entrySet()) {
...
emplist.put(e2.id,e2);

			
		}System.out.println("%d employes are imported"+countr);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
